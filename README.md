...
- Для оповещений о включении/выключении/перезагрузке ВМ укажите `AUDIT_CHANNEL_ID` (бот должен быть администратором канала/группы).
- Чтобы в аудит-уведомлениях выводились телефоны, задайте `USER_PHONES` в формате `123456789:+79990000000,987654321:+78880000000`.
- Чтобы фиксировать действия, выполненные напрямую в vCenter (например, power-on в vSphere Client), задайте `VC_EVENT_TYPES` (список типов событий через запятую, например `VmPoweredOnEvent,VmPoweredOffEvent,VmRebootingEvent`). Дополнительно можно настроить `VC_EVENT_POLL_INTERVAL`, `VC_EVENT_BATCH_SIZE` и `VC_EVENT_USER_IGNORE` (список vCenter-пользователей, которых нужно скрыть; учётка бота игнорируется автоматически).
